// Switch to the plp_bookstore database "use plp_bookstore"

// Find all books in a specific genre
db.books.find({genre: "Fiction" })
// Find books published after a certain year
db.books.find({published_year: {$gt: 1937}})
// Find books by a specific author
db.books.find({author: "Harper Lee"})
// Update the price of a specific book
db.books.updateOne({title: "Pride and Prejudice"}, {$set: {price: 9.99}})
// Delete a book by its title
db.books.deleteOne({title: "Brave New World"})

// Advanced queries
// Find books that are both in stock and published after 2010
db.books.find({in_stock: true, published_year: {$gt: 2010}})

// projection to return only the title, author, and price fields
db.books.find({genre: "Fiction"}, {title: 1, author: 1, price: 1, _id: 0})

// Sorting by price ascending
db.books.find().sort({price: 1})

// Sorting by price descending
db.books.find().sort({price: -1})

// Using the `limit` and `skip` methods to implement pagination (5 books per page)
db.books.find().skip(0).limit(5) // Page 1
db.books.find().skip(5).limit(5) // Page 2

// Aggregation
// The average price of books in each genre
db.books.aggregate([
  {$group: {_id: "$genre", averagePrice: {$avg: "$price"}}}
])

// The author with the most books in the collection
db.books.aggregate([
  {$group: {_id: "$author", bookCount: {$sum: 1}}},
  {$sort: {bookCount: -1}},
  {$limit: 1}
])

// Grouping books by publication decade and counting them
db.books.aggregate([
  {
    $group: {
      _id: {
        $concat: [
          {$toString: {$subtract: [{$year: "$published_date"}, {$mod: [{$year: "$published_date"}, 10]}]}},
          "s"
        ]
      },
      count: {$sum: 1}
    }
  },
  {$sort: {_id: 1}}
])


// Indexing
// index on the title field
db.books.createIndex({title: 1})

// Create a compound index on author and published_year
db.books.createIndex({author: 1, published_year: -1})

// Using the `explain()` method to demonstrate the performance improvement with indexes
db.books.find({title: "1984"}).explain("executionStats")


